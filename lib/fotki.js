...
var fotki = INHERIT({}, {
    ...
    helpers: {
        ...
        uploadAlbum: function(album) {
            return fotki.auth(credentials.yandexFotki.login, credentials.yandexFotki.token)
                .albums.getByName(album.title).then(function(albums) {
                    if(!albums.length) {
                        return fotki.albums.create({ title : album.title }).then(function() {
                            return fotki.albums.getByName(album.title);
                        }).then(function(albums) {
                                return recursiveArray(album.images, function(image) {
